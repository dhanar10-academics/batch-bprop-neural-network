...
public class BatchBpropNeuralNetwork {
	...
	public static void main(String[] args) {
		...
		BatchBpropNeuralNetwork bprop = new BatchBpropNeuralNetwork(2, 4, 1);
	...
	public boolean train(double data[][], double learningRate, double targetMse, double maxEpoch) {
		...
		while (true) {
			double dwInputHidden[][] = new double[yInput.length][yHidden.length];
			double dwHiddenOutput[][] = new double[yHidden.length][yOutput.length];
			...
			for (double[] d : data) {
				...
				for (int j = 0; j < yHidden.length; j++) {
					for (int k = 0; k < yInput.length; k++) {
						dwInputHidden[k][j] += learningRate * eHidden[j] * yInput[k];
					}
				}
				
				for (int j = 0; j < yOutput.length; j++) {
					for (int k = 0; k < yHidden.length; k++) {
						dwHiddenOutput[k][j] += learningRate * eOutput[j] * yHidden[k];
					}
				}
			...
			for (int j = 0; j < yOutput.length; j++) {
				for (int k = 0; k < yHidden.length; k++) {
					wHiddenOutput[k][j] += dwHiddenOutput[k][j];
				}
			}
